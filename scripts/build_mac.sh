...
# macOS: prefer g++ if installed via Homebrew, otherwise fall back to clang++
if command -v g++ &>/dev/null && g++ --version 2>&1 | grep -q "GCC"; then
    CXX=g++
else
    CXX=clang++
fi
...
# Compiler flags
# Note: -fno-omit-frame-pointer is supported by both GCC and Apple Clang
...
echo -e "${CYAN}============================================================${RESET}"
echo
echo -e "${BLUE}[*]${RESET} Compiler: ${BOLD}$CXX${RESET}"
...
for SRC in "${SOURCES[@]}"; do
    ...
    if [ "$NEEDS_COMPILE" -eq 1 ]; then
        ...
        # macOS mktemp requires a template argument
        ERR_FILE=$(mktemp /tmp/cfgen_build.XXXXXX)
        $CXX $CXXFLAGS -c "$SRC" -o "$OBJ" 2>"$ERR_FILE" &
        PIDS+=("$! $SRC $ERR_FILE")
    fi
done
